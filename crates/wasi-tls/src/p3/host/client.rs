...
impl HostConnectorWithStore for WasiTls {
        ...
        T: 'static,
    {
        ...
        Ok((rx, FutureReader::new(store, ResultProducer(err_rx))))
    }
        ...
        T: 'static,
    {
        store.with(|mut store| {
            ...
            let _ = send_cons_tx.send(PlaintextConsumer(Arc::clone(&stream)));
            let _ = send_prod_tx.send(CiphertextProducer(Arc::clone(&stream)));

            let _ = receive_cons_tx.send(CiphertextConsumer(Arc::clone(&stream)));
            let _ = receive_prod_tx.send(PlaintextProducer(Arc::clone(&stream)));
