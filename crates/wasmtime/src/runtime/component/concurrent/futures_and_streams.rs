...
/// Represents a pending `stream.forward` operation, held by both the source
/// stream's `ReadState::Forwarding` and the destination stream's
/// `WriteState::Forwarding` while the forward is pending.
#[derive(Copy, Clone)]
struct ForwardState {
    /// The component instance which registered the forward, owning both
    /// `src_handle` and `dst_handle`.
    instance: Instance,
    /// The source stream, whose write end supplies the items.
    src: TableId<TransmitState>,
    /// The destination stream, whose read end receives the items.
    dst: TableId<TransmitState>,
    /// The type of both streams.
    ty: TypeStreamTableIndex,
    /// The maximum number of items to forward.
    count: ItemCount,
    /// The number of items forwarded so far.
    forwarded: ItemCount,
    /// The forwarding guest's handle for the source stream's read end.
    src_handle: u32,
    /// The forwarding guest's handle for the destination stream's write end.
    dst_handle: u32,
    /// Whether the forwarding guest requested cancellation of this forward
    /// while a host rendezvous was in flight.
    cancel_forward: bool,
    /// Whether a `cancel_forward` request came in via the source handle, in
    /// which case the forward's terminal event is delivered on the source's
    /// read-handle waitable rather than the destination's write-handle
    /// waitable.
    cancel_via_src: bool,
...
/// through, which determines where the resulting completion is delivered.
#[derive(Copy, Clone)]
enum ForwardCancelVia {
    /// The forwarding guest cancelled the forward via the source's read
    /// handle.
    Src,
    /// The forwarding guest cancelled the forward via the destination's
    /// write handle.
    Dst,
    /// The guest peer cancelled its own operation grafted into the in-flight
    /// rendezvous.
    Peer,
}

/// Restore the forwarding guest's source handle entry from `Busy` back to
/// `Read`, with the specified done flag.
///
/// This happens eagerly, as soon as the forward's outcome is determined
/// (completion, teardown due to a peer drop, or a destination-initiated
/// cancellation), rather than when the `STREAM_FORWARD` event is delivered:
/// that event is queued on the destination's write-handle waitable, so there
/// is nothing to synchronize the restore with, and restoring eagerly keeps
/// the source stream usable while the event is still queued.  (For a
/// source-initiated cancellation the roles are swapped: the event is queued
/// on the source's read-handle waitable and `restore_forward_dst` eagerly
/// restores the destination handle entry instead.)
fn restore_forward_src(store: &mut StoreOpaque, fwd: &ForwardState, done: bool) -> Result<()> {
    let component_instance = fwd.instance.id().get_mut(store);
    let runtime_instance = component_instance.component().types()[fwd.ty].instance;
    let (_, src_state) = component_instance.instance_states().0[runtime_instance]
        .handle_table()
        .stream_rep(fwd.ty, fwd.src_handle)?;
    if *src_state != TransmitLocalState::Busy {
        bail_bug!("expected source state to be busy");
    }
    *src_state = TransmitLocalState::Read { done };
    Ok(())
}

/// Restore the forwarding guest's destination handle entry from `Busy` back
/// to `Write`, used when a forward is torn down or cancelled via the source
/// handle and thus no event will be delivered on the destination handle.
fn restore_forward_dst(store: &mut StoreOpaque, fwd: &ForwardState) -> Result<()> {
    let component_instance = fwd.instance.id().get_mut(store);
    let runtime_instance = component_instance.component().types()[fwd.ty].instance;
    let (_, dst_state) = component_instance.instance_states().0[runtime_instance]
        .handle_table()
        .stream_rep(fwd.ty, fwd.dst_handle)?;
    if *dst_state != TransmitLocalState::Busy {
        bail_bug!("expected destination state to be busy");
    }
    *dst_state = TransmitLocalState::Write { done: false };
    Ok(())
}

/// Settle a forward: clear both `Forwarding` registrations, eagerly restore
/// the source handle entry (with `src_done` indicating whether the source's
/// writer is gone), and queue the terminal `STREAM_FORWARD` event on the
/// destination's write-handle waitable.  `dst_dropped` indicates whether the
/// destination's reader is gone, which delivery translates into the
/// destination handle's done flag.
fn finish_forward(
    store: &mut StoreOpaque,
    fwd: &ForwardState,
    code: ReturnCode,
    ...
    let dst_write_handle = state.get_mut(fwd.dst)?.write_handle.rep();
    if state.take_event(dst_write_handle)?.is_some() {
        bail_bug!("unexpected event queued on the destination of a pending `stream.forward`");
    }
    restore_forward_src(store, fwd, src_done)?;
    store.concurrent_state_mut()?.set_event(
        dst_write_handle,
        Event::StreamForward {
            code,
            pending: Some((fwd.ty, fwd.dst_handle)),
            src: false,
            dropped: dst_dropped,
        },
    )
...
/// Settle a forward whose cancellation was requested via the source handle:
/// clear both `Forwarding` registrations, eagerly restore the destination
/// handle entry, and queue the terminal `STREAM_FORWARD` event on the
/// source's read-handle waitable, mirroring `finish_forward` with the roles
/// of the two handles swapped.
fn finish_forward_via_src(
    store: &mut StoreOpaque,
    fwd: &ForwardState,
    code: ReturnCode,
) -> Result<()> {
    let state = store.concurrent_state_mut()?;
    state.clear_forward(fwd.src, fwd.dst)?;
    let src_read_handle = state.get_mut(fwd.src)?.read_handle.rep();
    if state.take_event(src_read_handle)?.is_some() {
        bail_bug!("unexpected event queued on the source of a pending `stream.forward`");
    }
    restore_forward_dst(store, fwd)?;
    store.concurrent_state_mut()?.set_event(
        src_read_handle,
        Event::StreamForward {
            code,
            pending: Some((fwd.ty, fwd.src_handle)),
            src: true,
            dropped: false,
        },
    )
...
/// Record the progress a rendezvous made against a pending forward: settle
/// the forward with a completion event if its budget is now exhausted, or
/// re-register it with the updated count otherwise.
fn settle_forward_progress(
    store: &mut StoreOpaque,
    fwd: &ForwardState,
    copied: ItemCount,
) -> Result<()> {
    let forwarded = fwd.forwarded.add(copied)?;
    if forwarded == fwd.count {
        finish_forward(store, fwd, ReturnCode::Completed(fwd.count), false, false)
    } else {
        store
            .concurrent_state_mut()?
            .register_forward(ForwardState { forwarded, ..*fwd })
    }
}

/// Result of a `forward_copy` rendezvous between a pending write on the
/// source stream and a pending read on the destination stream of a
/// `stream.forward`.
struct ForwardCopyOutcome {
    /// The number of items copied, clamped to the forward's remaining budget.
    copied: ItemCount,
    /// Whether the write operation completed per the zero-length-read/write
    /// rules; see the comment in `guest_write`'s `ReadState::GuestReady` arm.
    write_complete: bool,
    /// Whether the read operation completed per the same rules.
    read_complete: bool,
}

/// Identifies the live guest party, if any, of a `stream.forward` rendezvous
/// with a host-owned endpoint, i.e. the party whose call triggered the
/// rendezvous and which receives its result as a return code rather than an
/// event.
#[derive(Copy, Clone, PartialEq)]
enum ForwardParty {
    /// The forwarding guest itself, via `stream.forward`.
    Forward,
    /// The guest peer: the source's writer or the destination's reader.
    Peer,
    /// No live guest party: the rendezvous was host-initiated or is being
    /// completed in the background.
    None,
}

/// Complete a rendezvous between the host producer owning the source's write
/// end and a guest read on the destination of the specified forward.
///
/// `read_state` must be the original (unclamped) `ReadState::GuestReady`
/// detached from the destination when the rendezvous began, `stale` the item
/// count of the read's accumulated completion event deferred when the
/// rendezvous began, and `code` the result of `settle_host_write` for the
/// batch. Routes the reader's result (directly, as an event, or by re-parking
/// the read), then settles or re-registers the forward. Returns the live
/// party's code plus the done flag for the forwarding guest's source handle.
fn finish_forward_produce(
    store: &mut StoreOpaque,
    fwd: &ForwardState,
    ...
    let ReadState::GuestReady {
        ...
        bail_bug!("expected `ReadState::GuestReady`; got `{read_state:?}`");
    };
    let (read_ty, read_handle) = (*read_ty, *read_handle);
    let (ReturnCode::Completed(copied)
    | ReturnCode::Dropped(copied)
    | ReturnCode::Cancelled(copied)) = code
    else {
        bail_bug!("unexpected return code: {code:?}");
    };

    let state = store.concurrent_state_mut()?;
    let (cancel_forward, cancel_via_src, cancel_peer) = match &state.get_mut(fwd.dst)?.write {
        WriteState::Forwarding(f) => (f.cancel_forward, f.cancel_via_src, f.cancel_peer),
        state => bail_bug!("expected `WriteState::Forwarding`; got `{state:?}`"),
    };

    // Route the reader's result, accumulating the deferred completion event,
    // if any. The reader never observes the producer's `DROPPED` directly:
    // its own peer is the forwarding guest, which still holds the
    // destination's write end, so a terminal producer merely completes the
    // read with whatever items it delivered, or leaves the read parked when
    // there are none.
    let reader_code = if cancel_peer {
        Some(ReturnCode::Cancelled(copied.add(stale)?))
    } else if copied > 0 || matches!(code, ReturnCode::Completed(_)) {
        Some(ReturnCode::completed(
            TransmitKind::Stream,
            copied.add(stale)?,
        ))
    } else {
    ...
    match (reader_code, live) {
        (Some(code), ForwardParty::Forward | ForwardParty::None) => {
            state.send_read_result(read_ty, fwd.dst, read_handle, code)?;
        }
        (Some(_), ForwardParty::Peer) => {}
        (None, _) => {
            state.get_mut(fwd.dst)?.read = read_state;
            if stale > 0 {
                state.send_read_result(
                    read_ty,
                    fwd.dst,
                    read_handle,
                    ReturnCode::completed(TransmitKind::Stream, stale),
                )?;
            }
        }
    }

    // Settle the forward itself.
    let forwarded = fwd.forwarded.add(copied)?;
    let fwd = ForwardState {
        forwarded,
        cancel_forward: false,
        cancel_via_src: false,
        cancel_peer: false,
        ..*fwd
    };
    let (forward_code, src_done) = if cancel_forward {
        ...
        match code {
            ...
            ReturnCode::Completed(_) | ReturnCode::Cancelled(_) => {
                ...
                let code = match live {
                    ForwardParty::Peer => reader_code.unwrap_or(ReturnCode::Blocked),
                    _ => ReturnCode::Blocked,
                };
                return Ok((code, false));
            }
            ReturnCode::Dropped(_) => (ReturnCode::Dropped(forwarded), true),
            ReturnCode::Blocked => unreachable!(),
        }
    };
    if let ForwardParty::Forward = live {
        store
            .concurrent_state_mut()?
            .clear_forward(fwd.src, fwd.dst)?;
        Ok((forward_code, src_done))
    } else {
        if cancel_forward && cancel_via_src {
            finish_forward_via_src(store, &fwd, forward_code)?;
        } else {
            finish_forward(store, &fwd, forward_code, false, src_done)?;
        }
        Ok((reader_code.unwrap_or(ReturnCode::Blocked), false))
    }
}

/// Complete a rendezvous between a guest write on the source and the host
/// consumer owning the destination's read end of the specified forward.
///
/// `write_state` must be the original (unclamped) `WriteState::GuestReady`
/// detached from the source when the rendezvous began, `stale` the item count
/// of the write's accumulated completion event deferred when the rendezvous
/// began, and `code` the result of `settle_host_read` for the batch. Routes
/// the writer's result, then settles or re-registers the forward. Returns the
/// live party's code plus the done flag for the forwarding guest's
/// destination handle.
fn finish_forward_consume(
    store: &mut StoreOpaque,
    fwd: &ForwardState,
    ...
    let WriteState::GuestReady {
        ...
        bail_bug!("expected `WriteState::GuestReady`; got `{write_state:?}`");
    };
    let (write_ty, write_handle) = (*write_ty, *write_handle);
    let (ReturnCode::Completed(copied)
    | ReturnCode::Dropped(copied)
    | ReturnCode::Cancelled(copied)) = code
    else {
    ...
    let state = store.concurrent_state_mut()?;
    let (cancel_forward, cancel_via_src, cancel_peer) = match &state.get_mut(fwd.src)?.read {
        ReadState::Forwarding(f) => (f.cancel_forward, f.cancel_via_src, f.cancel_peer),
        state => bail_bug!("expected `ReadState::Forwarding`; got `{state:?}`"),
    };

    // Route the writer's result, accumulating the deferred completion event,
    // if any; like the reader in `finish_forward_produce`, the writer never
    // observes the consumer's `DROPPED` directly.
    let writer_code = if cancel_peer {
        Some(ReturnCode::Cancelled(copied.add(stale)?))
    } else if copied > 0 || matches!(code, ReturnCode::Completed(_)) {
        Some(ReturnCode::completed(
            TransmitKind::Stream,
    ...
    match (writer_code, live) {
        (Some(code), ForwardParty::Forward | ForwardParty::None) => {
            state.send_write_result(write_ty, fwd.src, write_handle, code)?;
        }
        (Some(_), ForwardParty::Peer) => {}
        (None, _) => {
            state.get_mut(fwd.src)?.write = write_state;
            if stale > 0 {
                state.send_write_result(
                    write_ty,
                    fwd.src,
                    write_handle,
                    ReturnCode::completed(TransmitKind::Stream, stale),
    ...
    let (forward_code, dst_done) = if cancel_forward {
        (ReturnCode::Cancelled(forwarded), false)
    } else {
        match code {
            // The forward's budget is exhausted, so it completes even if this
            ...
            ReturnCode::Completed(_) | ReturnCode::Cancelled(_) => {
                store.concurrent_state_mut()?.register_forward(fwd)?;
                let code = match live {
                    ForwardParty::Peer => writer_code.unwrap_or(ReturnCode::Blocked),
                    _ => ReturnCode::Blocked,
                };
                return Ok((code, false));
            }
    ...
    if let ForwardParty::Forward = live {
        store
            .concurrent_state_mut()?
            .clear_forward(fwd.src, fwd.dst)?;
        Ok((forward_code, dst_done))
    } else {
        if cancel_forward && cancel_via_src {
            finish_forward_via_src(store, &fwd, forward_code)?;
        } else {
            finish_forward(store, &fwd, forward_code, dst_done, false)?;
        }
        Ok((writer_code.unwrap_or(ReturnCode::Blocked), false))
    }
}
...
enum WriteState {
    ...
    /// A `stream.forward` is pending, ready to accept items written to the
    /// source stream.
...
impl fmt::Debug for WriteState {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::Forwarding(_) => f.debug_tuple("Forwarding").finish(),
...
enum ReadState {
    ...
    /// A `stream.forward` is pending, ready to send items read from the
    /// destination stream.
    Forwarding(ForwardState),
...
impl StoreOpaque {
    ...
    /// Complete a pending host-produce rendezvous for a `stream.forward` in
    /// the background: settle the producer's batch on the source, then route
    /// the results via `finish_forward_produce`.
    fn pipe_forward_to_guest(
        &mut self,
        fwd: ForwardState,
        read_state: ReadState,
        stale: ItemCount,
        ...
        let future = async move {
            let stream_state = future.await?;
            tls::get(|store| {
                let transmit = store.concurrent_state_mut()?.get_mut(fwd.src)?;
                let code = settle_host_write(transmit, TransmitKind::Stream, stream_state)?;
                transmit.read = ReadState::Open;
                finish_forward_produce(store, &fwd, read_state, stale, code, ForwardParty::None)?;
                Ok(())
            })
        };
    ...
    /// Complete a pending host-consume rendezvous for a `stream.forward` in
    /// the background: settle the consumer's batch on the destination, then
    /// route the results via `finish_forward_consume`.
    fn pipe_forward_from_guest(
        &mut self,
        fwd: ForwardState,
        write_state: WriteState,
        stale: ItemCount,
        ...
        let future = async move {
            let stream_state = future.await?;
            tls::get(|store| {
                let transmit = store.concurrent_state_mut()?.get_mut(fwd.dst)?;
                let code = settle_host_read(transmit, TransmitKind::Stream, stream_state)?;
                transmit.write = WriteState::Open;
                finish_forward_consume(store, &fwd, write_state, stale, code, ForwardParty::None)?;
                Ok(())
            })
        };
    ...
    /// If the read side of the specified transmit currently holds a guest read
    /// grafted from the destination of a pending `stream.forward` (i.e. the
    /// transmit is the source of a forward with a host-produce rendezvous in
    /// flight), return that forward's state.
    fn forward_produce_graft(
        &mut self,
        id: TableId<TransmitState>,
    ) -> Result<Option<ForwardState>> {
        let state = self.concurrent_state_mut()?;
        let transmit = state.get_mut(id)?;
        if !matches!(&transmit.write, WriteState::HostReady { .. }) {
            return Ok(None);
        }
        let own = transmit.read_handle.rep();
        let &ReadState::GuestReady {
            instance,
            ty,
            handle,
        ...
        let (rep, _) = instance.id().get_mut(self).get_mut_by_index(ty, handle)?;
        if rep == own {
            return Ok(None);
        }
        let state = self.concurrent_state_mut()?;
        let dst = state.get_mut(TableId::<TransmitHandle>::new(rep))?.state;
        match &state.get_mut(dst)?.write {
            &WriteState::Forwarding(fwd) if fwd.src == id => Ok(Some(fwd)),
            state => bail_bug!("expected `WriteState::Forwarding`; got `{state:?}`"),
        }
    }

    /// If the write side of the specified transmit currently holds a guest
    /// write grafted from the source of a pending `stream.forward` (i.e. the
    /// transmit is the destination of a forward with a host-consume rendezvous
    /// in flight), return that forward's state.
    fn forward_consume_graft(
        &mut self,
        id: TableId<TransmitState>,
    ) -> Result<Option<ForwardState>> {
        let state = self.concurrent_state_mut()?;
        let transmit = state.get_mut(id)?;
        if !matches!(&transmit.read, ReadState::HostReady { .. }) {
            return Ok(None);
        }
        let own = transmit.write_handle.rep();
        let &WriteState::GuestReady {
            instance,
            ty,
            handle,
        ...
        let state = self.concurrent_state_mut()?;
        let src = state.get_mut(TableId::<TransmitHandle>::new(rep))?.state;
        match &state.get_mut(src)?.read {
    ...
    /// specified forward, recording which handle the request came in through
    ...
    fn request_forward_cancel(&mut self, fwd: &ForwardState, via: ForwardCancelVia) -> Result<()> {
        let set = |f: &mut ForwardState| match via {
            ForwardCancelVia::Src => {
                f.cancel_forward = true;
                f.cancel_via_src = true;
            }
            ForwardCancelVia::Dst => f.cancel_forward = true,
            ForwardCancelVia::Peer => f.cancel_peer = true,
        };
        let state = self.concurrent_state_mut()?;
        if let WriteState::HostReady {
            cancel,
            cancel_waker,
            ..
        } = &mut state.get_mut(fwd.src)?.write
        {
            *cancel = true;
            if let Some(waker) = cancel_waker.take() {
                waker.wake();
            }
            let WriteState::Forwarding(f) = &mut state.get_mut(fwd.dst)?.write else {
                bail_bug!("expected `WriteState::Forwarding`");
            };
            set(f);
            ...
            cancel_waker,
            ..
        } = &mut state.get_mut(fwd.dst)?.read
        {
            *cancel = true;
            if let Some(waker) = cancel_waker.take() {
                waker.wake();
            }
            let ReadState::Forwarding(f) = &mut state.get_mut(fwd.src)?.read else {
                bail_bug!("expected `ReadState::Forwarding`");
            };
            set(f);
        } else {
            bail_bug!("no host rendezvous in flight for forward");
        }
        Ok(())
    }
    ...
    fn host_drop_reader(&mut self, id: TableId<TransmitHandle>, kind: TransmitKind) -> Result<()> {
        ...
        if let ReadState::Forwarding(_) = &transmit.read {
            bail_bug!("cannot drop the read end of the source of a pending `stream.forward`")
        }
        ...
        match mem::replace(&mut transmit.write, new_state) {
            ...
            WriteState::Forwarding(fwd) => {
                finish_forward(self, &fwd, ReturnCode::Dropped(fwd.forwarded), true, false)?;
            }
        }
        Ok(())
    }
    ...
    fn host_drop_writer(
        ...
        match &mut transmit.write {
            ...
            WriteState::Forwarding(_) => {
                bail_bug!("cannot drop the write end of a stream with a pending `stream.forward`")
            }
        ...
        match mem::replace(&mut transmit.read, new_state) {
            ...
            ReadState::Forwarding(fwd) => {
                finish_forward(self, &fwd, ReturnCode::Dropped(fwd.forwarded), false, true)?;
            }
        }
        Ok(())
    }
            ...
            WriteState::Forwarding(fwd) => {
                ...
                match &src_tx.write {
                    ...
                    WriteState::GuestReady { .. } if fwd.forwarded < fwd.count => {
                        let write_state = mem::replace(&mut src_tx.write, WriteState::Open);
                        Instance::forward_consume(
                            self.as_context_mut(),
                            &fwd,
                            consume,
                            write_state,
                    ...
                    WriteState::GuestReady { .. } | WriteState::Open => {
                        let probe = matches!(&src_tx.write, WriteState::GuestReady { .. });
                        ...
                        if probe {
                            // A zero-budget forward is a readiness probe which
                            // the parked write and this consumer jointly
                            // satisfy.
                            finish_forward(
                                self.0,
                                &fwd,
                    ...
                    state => bail_bug!(
                        "unexpected write state for the source of a pending \
                         `stream.forward`: {state:?}"
                    ),
                }
            }
            WriteState::Dropped => {
...
impl Instance {
    ...
    /// Copy items directly from a pending write on the source stream of the
    /// specified forward to a pending read on its destination stream,
    /// clamping the amount copied to the forward's remaining budget.
    ///
    /// `write_state` and `read_state` must be the `GuestReady` states taken
    /// from the source's write side and the destination's read side,
    /// respectively. A side whose `notify_*` parameter is true is a parked
    /// third party: its completion event is queued (accumulating into any
    /// previously queued completion) and its unconsumed buffer remainder is
    /// re-registered. A side whose `notify_*` parameter is false is the live
    /// caller, which receives its result via this function's return value
    /// instead.
    fn forward_copy<T: 'static>(
        ...
        if write_flat_abi != read_flat_abi {
            bail_bug!("expected flat ABI calculations to be the same");
        }

        let write_runtime_instance = write_instance.runtime_instance(write_caller);
        let read_runtime_instance = read_instance.runtime_instance(read_caller_instance);

        // Apply the intra-component copy restriction eagerly, with a
        // forward-specific error, before either parked operation is consumed
        // by the copy below.
        {
            let types = write_instance.id().get(store.0).component().types();
            if write_runtime_instance == read_runtime_instance
                && !allow_intra_component_read_write(write_ty.payload(types))
            {
                bail!(
                    "cannot `stream.forward` between intra-component streams with non-numeric \
                     payload"
                )
            }
        }

        // A zero-length forward is a readiness probe: it completes once a
        // write and a read are simultaneously parked, without consuming or
        // completing either of them, so the zero-length read/write rules do
        // not apply to the parked operations.
        let budget = fwd.count.sub(fwd.forwarded)?;
        let (write_complete, read_complete) = if budget == 0 {
            (false, false)
        } else {
            // See the comment in `guest_write`'s `ReadState::GuestReady` arm
            // concerning zero-length reads and writes.
            (write_count == 0 || read_count > 0, write_count > 0)
        };

        let copied = write_count.min(read_count).min(budget);
        ...
        let write_item_size = {
            let types = write_instance.id().get(store.0).component().types();
            match write_ty.payload(types) {
                Some(ty) => usize::try_from(types.canonical_abi(ty).size32)?,
                None => 0,
            }
        };
        let read_item_size = {
            let types = read_instance.id().get(store.0).component().types();
            match read_ty.payload(types) {
                Some(ty) => usize::try_from(types.canonical_abi(ty).size32)?,
                None => 0,
            }
        };
        ...
        if notify_writer && write_complete {
            let write_handle_rep = concurrent_state.get_mut(fwd.src)?.write_handle.rep();
            let total = if let Some(Event::StreamWrite {
                code: ReturnCode::Completed(old_total),
                ..
            }) = concurrent_state.take_event(write_handle_rep)?
            {
                copied.add(old_total)?
            } else {
                copied
            };
            concurrent_state.send_write_result(
                ...
                fwd.src,
                write_handle,
                ReturnCode::completed(write_ty.kind(), total),
            )?;
        }

        if notify_reader && read_complete {
            let read_handle_rep = concurrent_state.get_mut(fwd.dst)?.read_handle.rep();
            ...
            concurrent_state.send_read_result(
                read_ty,
                fwd.dst,
                read_handle,
                ReturnCode::completed(read_ty.kind(), total),
            )?;
        }

        // Restore the writer if it has buffer remaining, or if it parked a
        // zero-length write which a zero-budget probe left uncompleted.
        if notify_writer && (copied < write_count || (write_count == 0 && !write_complete)) {
            let src_tx = concurrent_state.get_mut(fwd.src)?;
            src_tx.write = WriteState::GuestReady {
                instance: write_instance,
                caller: write_caller,
                ty: write_ty,
                flat_abi: write_flat_abi,
                options: write_options,
                address: write_address + (copied.as_usize() * write_item_size),
                count: write_count.sub(copied)?,
                handle: write_handle,
            };
        }

        // If the reader still has buffer remaining, or if this was a
        // zero-length rendezvous, then restore the state of the reader back
        // to what it was when we found it. Note that for the zero-length
        // rendezvous case this specifically won't execute the `read_complete`
        // logic above, which is intentional, as the reader remains blocked.
        if notify_reader && (copied < read_count || (copied == 0 && read_count == 0)) {
            let dst_tx = concurrent_state.get_mut(fwd.dst)?;
            dst_tx.read = ReadState::GuestReady {
                ty: read_ty,
                flat_abi: read_flat_abi,
                options: read_options,
                address: read_address + (copied.as_usize() * read_item_size),
                count: read_count.sub(copied)?,
                handle: read_handle,
                instance: read_instance,
                caller_instance: read_caller_instance,
                caller_thread: read_caller_thread,
    ...
    /// Rendezvous the host producer owning the source's write end of the
    /// specified forward with a guest read on its destination.
    ///
    /// `produce` and `try_into` must have been detached from the source's
    /// `WriteState::HostReady` and `read_state` (a `ReadState::GuestReady`)
    /// from the destination's read side. The read is grafted onto the source,
    /// clamped to the forward's remaining budget, so the single-transmit host
    /// `produce` machinery can deliver items directly to the reader's buffer.
    /// If the producer is not immediately ready the rendezvous completes in
    /// the background and the live party observes `BLOCKED`.
    fn forward_produce<T: 'static>(
        store: StoreContextMut<T>,
        fwd: &ForwardState,
        produce: PollStream,
        try_into: TryInto,
        ...
        // Defer the read's accumulated completion event, if any, for the
        // duration of the rendezvous: delivering it while the producer may be
        // writing into the read's buffer would prematurely release that
        // buffer to the guest. Its item count is folded into the read's
        // result when the rendezvous settles.
        let read_handle_rep = state.get_mut(fwd.dst)?.read_handle.rep();
        let stale = if let Some(Event::StreamRead {
            code: ReturnCode::Completed(count),
            ..
        }) = state.take_event(read_handle_rep)?
        {
            count
        } else {
            ItemCount::ZERO
        ...
        let src_tx = state.get_mut(fwd.src)?;
        match &src_tx.read {
            ReadState::Open | ReadState::Forwarding(_) => {}
            state => bail_bug!("unexpected source read state: {state:?}"),
        }
        src_tx.read = ReadState::GuestReady {
            ty,
            caller_instance,
            caller_thread,
            flat_abi,
        ...
        let dst_tx = state.get_mut(fwd.dst)?;
        match &dst_tx.write {
            WriteState::Open | WriteState::Forwarding(_) => {}
            state => bail_bug!("unexpected destination write state: {state:?}"),
        }
        dst_tx.write = WriteState::Forwarding(*fwd);

        let mut future = produce();
        state.get_mut(fwd.src)?.write = WriteState::HostReady {
            produce,
            try_into,
            guest_offset: ItemCount::ZERO,
            cancel: false,
        ...
        match poll {
            Poll::Ready(stream_state) => {
                let transmit = store.0.concurrent_state_mut()?.get_mut(fwd.src)?;
                let code = settle_host_write(transmit, TransmitKind::Stream, stream_state?)?;
                transmit.read = ReadState::Open;
                finish_forward_produce(store.0, fwd, read_state, stale, code, live)
            }
            Poll::Pending => {
                store
                    .0
                    .pipe_forward_to_guest(*fwd, read_state, stale, future);
                Ok((ReturnCode::Blocked, false))
            }
        }
    }

    /// Rendezvous a guest write on the source of the specified forward with
    /// the host consumer owning the destination's read end.
    ///
    /// `consume` must have been detached from the destination's
    /// `ReadState::HostReady` and `write_state` (a `WriteState::GuestReady`)
    /// from the source's write side. The write is grafted onto the
    /// destination, clamped to the forward's remaining budget, so the
    /// single-transmit host `consume` machinery can lift items directly from
    /// the writer's buffer. If the consumer is not immediately ready the
    /// rendezvous completes in the background and the live party observes
    /// `BLOCKED`.
    fn forward_consume<T: 'static>(
        store: StoreContextMut<T>,
        fwd: &ForwardState,
        consume: PollStream,
        write_state: WriteState,
        ...
        // Defer the write's accumulated completion event, if any, for the
        // duration of the rendezvous: delivering it while the consumer may be
        // reading from the write's buffer would prematurely release that
        // buffer to the guest. Its item count is folded into the write's
        // result when the rendezvous settles.
        let write_handle_rep = state.get_mut(fwd.src)?.write_handle.rep();
        let stale = if let Some(Event::StreamWrite {
            code: ReturnCode::Completed(count),
            ..
        }) = state.take_event(write_handle_rep)?
        {
            count
        } else {
            ItemCount::ZERO
        ...
        src_tx.read = ReadState::Forwarding(*fwd);
        let dst_tx = state.get_mut(fwd.dst)?;
        match &dst_tx.write {
            WriteState::Open | WriteState::Forwarding(_) => {}
            state => bail_bug!("unexpected destination write state: {state:?}"),
        }
        dst_tx.write = WriteState::GuestReady {
            instance,
            caller,
            ty,
            flat_abi,
        ...
        let mut future = consume();
        state.get_mut(fwd.dst)?.read = ReadState::HostReady {
            consume,
            guest_offset: ItemCount::ZERO,
            cancel: false,
            cancel_waker: None,
        ...
        match poll {
            Poll::Ready(stream_state) => {
                let transmit = store.0.concurrent_state_mut()?.get_mut(fwd.dst)?;
                let code = settle_host_read(transmit, TransmitKind::Stream, stream_state?)?;
                transmit.write = WriteState::Open;
                finish_forward_consume(store.0, fwd, write_state, stale, code, live)
            }
            Poll::Pending => {
                store
                    .0
                    .pipe_forward_from_guest(*fwd, write_state, stale, future);
                Ok((ReturnCode::Blocked, false))
            }
        }
    }
    ...
    pub(super) fn guest_write<T: 'static>(
        ...
        *state = TransmitLocalState::Busy;
        ...
        let mut result = match mem::replace(&mut transmit.read, new_state) {
            ...
            ReadState::Forwarding(fwd) => {
                let dst_tx = concurrent_state.get_mut(fwd.dst)?;
                if matches!(&dst_tx.read, ReadState::GuestReady { .. }) {
                    let read_state = mem::replace(&mut dst_tx.read, ReadState::Open);
                    dst_tx.write = WriteState::Open;

                    let write_state = WriteState::GuestReady {
                        instance: self,
                        caller,
                        ty,
                        flat_abi,
                    ...
                    let outcome = Instance::forward_copy(
                        store.as_context_mut(),
                        &fwd,
                        write_state,
                        read_state,
                    ...
                    settle_forward_progress(store.0, &fwd, outcome.copied)?;

                    let concurrent_state = store.0.concurrent_state_mut()?;
                    if outcome.write_complete {
                        ReturnCode::completed(ty.kind(), outcome.copied)
                    } else {
                        set_guest_ready(concurrent_state)?;
                        ReturnCode::Blocked
                    }
                } else if matches!(&dst_tx.read, ReadState::HostReady { .. }) {
                    if fwd.forwarded == fwd.count {
                        // A zero-budget forward is a readiness probe which the
                        // host consumer and this write jointly satisfy; settle
                        // the forward, then park the write normally.
                        finish_forward(
                            store.0,
                            &fwd,
                            ReturnCode::Completed(fwd.count),
                        ...
                        let ReadState::HostReady {
                            ...
                            cancel,
                            cancel_waker,
                        } = mem::replace(&mut dst_tx.read, ReadState::Open)
                        else {
                            unreachable!()
                        };
                        if cancel_waker.is_some() || cancel || guest_offset != 0 {
                            bail_bug!("unexpected consumer state");
                        }
                        let write_state = WriteState::GuestReady {
                            instance: self,
                            caller,
                        ...
                        let (code, _) = Instance::forward_consume(
                            store.as_context_mut(),
                            &fwd,
                            consume,
                            write_state,
                    ...
                    match &dst_tx.read {
                        ReadState::Open => {}
                        state => bail_bug!(
                            "unexpected read state for the destination of a pending \
                             `stream.forward`: {state:?}"
                        ),
                    }
                    concurrent_state.register_forward(fwd)?;
                    set_guest_ready(concurrent_state)?;
                    ReturnCode::Blocked
                }
            }
    ...
    pub(super) fn guest_read<T: 'static>(
        ...
        *state = TransmitLocalState::Busy;
        ...
        let mut result = match mem::replace(&mut transmit.write, new_state) {
            ...
            WriteState::Forwarding(fwd) => {
                let src_tx = concurrent_state.get_mut(fwd.src)?;
                if matches!(&src_tx.write, WriteState::GuestReady { .. }) {
                    let write_state = mem::replace(&mut src_tx.write, WriteState::Open);
                    concurrent_state.clear_forward(fwd.src, fwd.dst)?;
                    let read_state = ReadState::GuestReady {
                        ty,
                        flat_abi,
                        options,
                        address,
                    ...
                    let concurrent_state = store.0.concurrent_state_mut()?;
                    if outcome.read_complete {
                        ReturnCode::completed(ty.kind(), outcome.copied)
                    } else {
                        set_guest_ready(concurrent_state)?;
                        ReturnCode::Blocked
                    }
                } else if matches!(&src_tx.write, WriteState::HostReady { .. }) {
                    if fwd.forwarded == fwd.count {
                        // A zero-budget forward is a readiness probe which the
                        // host producer and this read jointly satisfy; settle
                        // the forward, then park the read normally.
                        finish_forward(
                            store.0,
                            &fwd,
                            ReturnCode::Completed(fwd.count),
                        ...
                        let WriteState::HostReady {
                            ...
                            cancel,
                            cancel_waker,
                        } = mem::replace(&mut src_tx.write, WriteState::Open)
                        else {
                            unreachable!()
                        };
                        if cancel_waker.is_some() || cancel || guest_offset != 0 {
                            bail_bug!("unexpected producer state");
                        }
                        let read_state = ReadState::GuestReady {
                            ty,
                            flat_abi,
                        ...
                        let (code, _) = Instance::forward_produce(
                            store.as_context_mut(),
                            &fwd,
                            produce,
                            try_into,
                    ...
                    // The forward stays registered; restore the destination's
                    // write side taken by the enclosing match.
                    concurrent_state.get_mut(transmit_id)?.write = WriteState::Forwarding(fwd);
                    set_guest_ready(concurrent_state)?;
                    ReturnCode::Blocked
                }
            }
    ...
    pub(super) fn guest_forward<T: 'static>(
        ...
        let transmit_ty = TransmitIndex::Stream(ty);
        let count = ItemCount::new(count)?;

        if !async_ {
            // The caller may only sync call `stream.forward` from an
            // async task (i.e. a task created via a call to an async export).
            // Otherwise, we'll trap.
            store.0.check_blocking()?;
        }

        let (src_rep, src_state) = self
            .id()
            .get_mut(store.0)
            .get_mut_by_index(transmit_ty, src)?;
        let TransmitLocalState::Read { done } = *src_state else {
            bail!(Trap::ConcurrentFutureStreamOp);
        };
        if done {
            bail!("cannot forward after being notified that the writable end dropped");
        }
        *src_state = TransmitLocalState::Busy;

        let (dst_rep, dst_state) = self
            .id()
            .get_mut(store.0)
            .get_mut_by_index(transmit_ty, dst)?;
        let TransmitLocalState::Write { done } = *dst_state else {
            bail!(Trap::ConcurrentFutureStreamOp);
        };
        if done {
            bail!("cannot forward after being notified that the readable end dropped");
        }
        *dst_state = TransmitLocalState::Busy;

        let src_tx_handle = TableId::<TransmitHandle>::new(src_rep);
        let dst_tx_handle = TableId::<TransmitHandle>::new(dst_rep);

        store.0.current_guest_thread()?;
        let concurrent_state = store.0.concurrent_state_mut()?;

        let src_tx_id = concurrent_state.get_mut(src_tx_handle)?.state;
        let dst_tx_id = concurrent_state.get_mut(dst_tx_handle)?.state;

        log::trace!(
            "guest_forward {count} from {src_tx_handle:?} (handle {src}; state {src_tx_id:?}) to {dst_tx_handle:?} (handle {dst}; state {dst_tx_id:?})",
        );

        if src_tx_id == dst_tx_id {
            bail!("cannot forward a stream to itself");
        }

        /// Classification of one side of a prospective forward, after
        /// rejecting the states the forward implementation does not support.
        enum PeerState {
            /// No operation is parked on this side.
            Open,
            /// A guest operation is parked on this side, ready to rendezvous.
            Ready,
            /// This side is owned by the host, which is ready to rendezvous.
            Host,
            /// This side's peer end has been dropped.
            Dropped,
        }

        let src_tx = concurrent_state.get_mut(src_tx_id)?;
        if src_tx.done {
            bail!("cannot forward from stream after previous read succeeded");
        }
        let src_peer = match &src_tx.write {
            WriteState::Forwarding(_) => bail!("chained `stream.forward` is not supported"),
            WriteState::HostReady { .. } => PeerState::Host,
            WriteState::Open => PeerState::Open,
            WriteState::GuestReady { .. } => PeerState::Ready,
            WriteState::Dropped => PeerState::Dropped,
        };

        let dst_tx = concurrent_state.get_mut(dst_tx_id)?;
        if dst_tx.done {
            bail!(
                "cannot forward to stream after previous write succeeded or readable end dropped"
            );
        }
        let dst_read_handle = dst_tx.read_handle;
        let dst_peer = match &dst_tx.read {
            ReadState::Forwarding(_) => bail!("chained `stream.forward` is not supported"),
            ReadState::HostToHost { .. } => bail_bug!("unexpected `ReadState::HostToHost`"),
            ReadState::HostReady { .. } => PeerState::Host,
            ReadState::Open => PeerState::Open,
            ReadState::GuestReady { .. } => PeerState::Ready,
            ReadState::Dropped => PeerState::Dropped,
        };

        // A destination whose readable end has been transferred to the host
        // without a consumer attached yet is treated like `Open`: if a
        // consumer is attached while the forward is pending, `set_consumer`
        // performs the rendezvous. Forwarding between two host-owned ends is
        // unsupported, however, so reject that combination eagerly.
        if matches!(src_peer, PeerState::Host)
            && matches!(dst_peer, PeerState::Open)
            && concurrent_state
        ...
        let result = match (src_peer, dst_peer) {
            (PeerState::Dropped, PeerState::Dropped) => {
                src_done = true;
                dst_done = true;
                ReturnCode::Dropped(ItemCount::ZERO)
            }
            ...
            (PeerState::Ready, PeerState::Ready) => {
                let src_tx = concurrent_state.get_mut(src_tx_id)?;
                let write_state = mem::replace(&mut src_tx.write, WriteState::Open);
                let dst_tx = concurrent_state.get_mut(dst_tx_id)?;
                let read_state = mem::replace(&mut dst_tx.read, ReadState::Open);
                ...
                if outcome.copied == count {
                    ReturnCode::Completed(count)
                } else {
                    store
                        .0
                        .concurrent_state_mut()?
                        .register_forward(ForwardState {
                            forwarded: outcome.copied,
                            ..fwd
                        })?;
                    ReturnCode::Blocked
                }
            ...
            (PeerState::Host, PeerState::Ready) => {
                if count == 0 {
                    // A zero-length forward is a readiness probe, which the
                    // host producer and the parked read jointly satisfy
                    // without consuming either.
                    ReturnCode::Completed(ItemCount::ZERO)
                } else {
                    let src_tx = concurrent_state.get_mut(src_tx_id)?;
                    let WriteState::HostReady {
                        produce,
                    ...
                    let dst_tx = concurrent_state.get_mut(dst_tx_id)?;
                    let read_state = mem::replace(&mut dst_tx.read, ReadState::Open);
                    let (code, done) = Instance::forward_produce(
                        store.as_context_mut(),
                        &fwd,
                        produce,
                        try_into,
                        read_state,
                        ForwardParty::Forward,
                    )?;
                    src_done = done;
                    code
                }
            }
            (PeerState::Ready, PeerState::Host) => {
                if count == 0 {
                    ReturnCode::Completed(ItemCount::ZERO)
                } else {
                    let dst_tx = concurrent_state.get_mut(dst_tx_id)?;
                    ...
                    let src_tx = concurrent_state.get_mut(src_tx_id)?;
                    let write_state = mem::replace(&mut src_tx.write, WriteState::Open);
                    let (code, done) = Instance::forward_consume(
                        store.as_context_mut(),
                        &fwd,
                        consume,
                        write_state,
            ...
            (PeerState::Open, _) | (_, PeerState::Open) => {
                concurrent_state.register_forward(fwd)?;
                ReturnCode::Blocked
            }
        };

        if result == ReturnCode::Blocked && !async_ {
            // `Waitable::on_delivery` restores the destination handle entry
            // (with a done flag derived from the delivered event) and the
            // source handle entry was restored when the forward settled, so
            // return the code directly rather than falling through to the
            // restore below.
            let result = self.wait_for_write(store.0, dst_tx_handle)?;
            log::trace!(
                "guest_forward result from {src_tx_handle:?} (handle {src}; state {src_tx_id:?}) to {dst_tx_handle:?} (handle {dst}; state {dst_tx_id:?}): {result:?}",
            );
            return Ok(result);
        }
        ...
        if result != ReturnCode::Blocked {
            *self
                ...
                .get_mut(store.0)
                .get_mut_by_index(transmit_ty, src)?
                .1 = TransmitLocalState::Read { done: src_done };
            *self
                .id()
                .get_mut(store.0)
                .get_mut_by_index(transmit_ty, dst)?
                .1 = TransmitLocalState::Write { done: dst_done };
        }
    ...
    fn wait_for_write(
        ...
        if let Some(
            event @ (Event::StreamWrite { code, .. }
            | Event::FutureWrite { code, .. }
            | Event::StreamForward { code, .. }),
        ) = event
        {
            waitable.on_delivery(store, self, event)?;
            Ok(code)
        } else {
            bail_bug!("expected a stream write, future write, or stream forward event")
        }
    }
    ...
    fn cancel_write(
        ...
        let code = if let Some(event) = waitable.take_event(state)? {
            let (Event::FutureWrite { code, .. }
            | Event::StreamWrite { code, .. }
            | Event::StreamForward { code, .. }) = event
            else {
            ...
            match (code, event) {
                ...
                // A queued `StreamForward` completion means the forward's
            ...
            // The forwarding guest is cancelling via the destination handle of
            ...
            &store.concurrent_state_mut()?.get_mut(transmit_id)?.write
        {
                ...
                && matches!(&src_tx.read, ReadState::GuestReady { .. })
            {
                ...
                store
                    .concurrent_state_mut()?
                    .clear_forward(fwd.src, fwd.dst)?;
                restore_forward_src(store, &fwd, false)?;
            ...
            cancel_waker,
            ..
        } = &mut store.concurrent_state_mut()?.get_mut(transmit_id)?.read
        {
            *cancel = true;
            if let Some(waker) = cancel_waker.take() {
                waker.wake();
            ...
            &store.concurrent_state_mut()?.get_mut(transmit_id)?.read
        {
            ...
            if matches!(
                ...
                store.request_forward_cancel(&fwd, ForwardCancelVia::Peer)?;
                if async_ {
                    ReturnCode::Blocked
                } else {
                    let handle = store
        ...
        if !matches!(code, ReturnCode::Blocked) {
            ...
            match &transmit.write {
                ...
                WriteState::Forwarding(_) => {
                    bail_bug!("`stream.forward` should have been torn down during cancellation")
                }
    ...
    fn wait_for_read(
        ...
        let event = waitable.take_event(store.concurrent_state_mut()?)?;
        if let Some(
            event @ (Event::StreamRead { code, .. }
            | Event::FutureRead { code, .. }
            | Event::StreamForward { code, .. }),
        ) = event
        {
            waitable.on_delivery(store, self, event)?;
            ...
            bail_bug!("expected a stream read, future read, or stream forward event")
        }
    }
    ...
    fn cancel_read(
        ...
        let code = if let Some(event) = waitable.take_event(state)? {
            let (Event::FutureRead { code, .. }
            | Event::StreamRead { code, .. }
            | Event::StreamForward { code, .. }) = event
            else {
                bail_bug!("expected a stream read, future read, or stream forward event")
            };
            ...
            // The forwarding guest is cancelling via the source handle of a
            ...
            &store.concurrent_state_mut()?.get_mut(transmit_id)?.read
        {
            ...
            cancel_waker,
            ..
        } = &mut store.concurrent_state_mut()?.get_mut(transmit_id)?.write
        {
            *cancel = true;
            if let Some(waker) = cancel_waker.take() {
                waker.wake();
...
impl ConcurrentState {
    /// Register the specified pending forward on both the source stream's read
    /// side and the destination stream's write side.
    ///
    /// Each side must either be idle (`Open`) or already hold a `Forwarding`
    /// state for the same source/destination pair; callers which have
    /// temporarily taken one side via `mem::replace` re-register both sides
    /// through this.
    fn register_forward(&mut self, fwd: ForwardState) -> Result<()> {
        let src_tx = self.get_mut(fwd.src)?;
        match &src_tx.read {
            ReadState::Open => {}
            ReadState::Forwarding(other) if other.src == fwd.src && other.dst == fwd.dst => {}
            state => bail_bug!(
                "expected `ReadState::Open` or a matching `ReadState::Forwarding`; got `{state:?}`"
            ),
        }
        src_tx.read = ReadState::Forwarding(fwd);

        let dst_tx = self.get_mut(fwd.dst)?;
        match &dst_tx.write {
            WriteState::Open => {}
            WriteState::Forwarding(other) if other.src == fwd.src && other.dst == fwd.dst => {}
            state => bail_bug!(
                "expected `WriteState::Open` or a matching `WriteState::Forwarding`; got `{state:?}`"
            ),
        }
        dst_tx.write = WriteState::Forwarding(fwd);
        Ok(())
    ...
    /// Clear a pending forward from both the source stream's read side and the
    /// destination stream's write side, tolerating that the caller may have
    /// already taken either side via `mem::replace`.
    fn clear_forward(
        &mut self,
        src: TableId<TransmitState>,
        dst: TableId<TransmitState>,
    ) -> Result<()> {
        let src_tx = self.get_mut(src)?;
        if let ReadState::Forwarding(_) = &src_tx.read {
            src_tx.read = ReadState::Open;
        }
        let dst_tx = self.get_mut(dst)?;
        if let WriteState::Forwarding(_) = &dst_tx.write {
            dst_tx.write = WriteState::Open;
        }
        Ok(())
    }
    ...
    fn update_event(&mut self, waitable: u32, event: Event) -> Result<()> {
        ...
        let event = match (waitable.take_event(self)?, event) {
            ...
            // A `StreamWrite` drop notice merged into a queued forward event
            // means the destination's reader dropped after the forward
            // settled; record that so delivery marks the destination handle
            // done.
            (
                Some(Event::StreamForward {
                    code: old_code,
                    pending,
                    src: false,
                    ..
            ...
            // Likewise for a `StreamRead` drop notice merged into a forward
            // event queued on the source's read handle: the source's writer
            // dropped after the forward settled.
            (
                Some(Event::StreamForward {
                    code: old_code,
                    pending,
...
impl Waitable {
    ...
    pub(super) fn on_delivery(
        ...
        let component_instance = instance.id().get_mut(store);
        let (rep, state, code) = match event {
            ...
            | Event::FutureWrite {
                ...
                let runtime_instance = component_instance.component().types()[ty].instance;
                let (rep, state) = component_instance.instance_states().0[runtime_instance]
                    .handle_table()
        ...
        *state = match event {
            ...
            // The handle entry this event was not routed to was already
            // restored when the forward settled; only the handle entry the
            // event was queued on is restored on delivery, marked done if its
            // peer end is gone.
            Event::StreamForward { src, dropped, .. } => {
                if src {
                    TransmitLocalState::Read { done: dropped }
                } else {
                    TransmitLocalState::Write { done: dropped }
                }
            }
